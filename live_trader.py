...
from common.tabdeal_broker import (  # noqa: E402
    ...
    discover_irt_margin_symbols, BrokerError, DRY_RUN,
)
...
def manage_open_position(state: dict, tabdeal_symbol: str):
    ...
    hit_tp2 = (price >= pos["tp2"]) if pos["direction"] == "long" else (price <= pos["tp2"])
    ...
    if DRY_RUN:
        ...
        pnl = paper_ledger.record_close(
            PAPER_LOG_FILE, now_iso(), tabdeal_symbol, "ICT/SMC v2", pos["direction"],
            pos["entry"], price, pos["quantity"], balance, reason,
        )
        new_balance = balance + pnl
        state["_paper_balance_irt"] = new_balance
        notify(
            f"⚪ [آزمایشی] بسته شدن پوزیشن {tabdeal_symbol} ({pos['direction']}) — دلیل: {reason}\n"
            f"قیمت خروج≈{price:,.0f} | PnL فرضی≈{pnl:,.0f} تومان | موجودی فرضی≈{new_balance:,.0f} تومان"
        )
    else:
        try:
            close_margin_position(tabdeal_symbol, opposite, pos["quantity"], logger=print)
        except BrokerError as e:
            notify(f"❌ خطا در بستن پوزیشن واقعی {tabdeal_symbol}: {e}")
            return
        notify(f"⚪ بسته شدن پوزیشن واقعی {tabdeal_symbol} ({pos['direction']}) — دلیل: {reason} — قیمت≈{price:,.0f}")
...
def main():
    ...
    for binance_symbol, tabdeal_symbol in binance_to_tabdeal.items():
        ...
        real_price = float(order.get("price", order.get("_planned_price", price)))
        ...
        state[tabdeal_symbol] = {
            ...
            "quantity": qty, "opened_at": str(candle_time),
